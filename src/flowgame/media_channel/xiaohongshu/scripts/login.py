...
QRCODE_DIR = os.path.join(SKILL_DIR, "data")
QRCODE_PATH = os.path.join(QRCODE_DIR, "qrcode.png")
# flowgame_python 仓库根：…/xiaohongshu/scripts → 上四级到仓库根
# scripts -> xiaohongshu -> media_channel -> flowgame -> src -> 不对
# Path: repo/src/flowgame/media_channel/xiaohongshu/scripts/login.py
# parents[0]=scripts, [1]=xiaohongshu, [2]=media_channel, [3]=flowgame, [4]=src, [5]=repo
_REPO_ROOT = os.path.abspath(os.path.join(SKILL_DIR, "..", "..", "..", ".."))


def qrcode_repo_relative(abs_path: Optional[str] = None) -> Optional[str]:
    """把绝对路径转成相对 flowgame_python 仓库根的路径，如 ``src/flowgame/.../qrcode.png``。"""
    path = abs_path or QRCODE_PATH
    if not path:
        return None
    abs_p = os.path.abspath(path)
    try:
        rel = os.path.relpath(abs_p, _REPO_ROOT)
    except ValueError:
        return abs_p
    # 统一用正斜杠，方便前端/跨平台
    return rel.replace("\\", "/")
...
def check_login(
    ...
    """检查登录状态。返回 ``(是否已登录, 用户名)``，不含二维码图片。

    需要二维码路径时请用 :func:`get_qrcode`。
    """
...
def get_qrcode(
    ...
    Returns:
        dict:
          - logged_in: bool
          - username: str | None
          - qrcode_path: 绝对路径（已登录时为 None）
          - qrcode_rel: 相对仓库根路径，如 ``src/flowgame/media_channel/xiaohongshu/data/qrcode.png``
          - message: str
    """
    ...
    try:
        client.start()
        action = LoginAction(client)
        qrcode_path, already = action.get_wechat_qrcode()
        if already:
            ok, username = action.check_login_status(navigate=False)
            return {
                "logged_in": True,
                "username": username,
                "qrcode_path": None,
        ...
        abs_path = qrcode_path
        if abs_path and copy_to:
            import shutil
        ...
        rel = qrcode_repo_relative(abs_path) if abs_path else None
        return {
            "logged_in": False,
            "username": None,
            "qrcode_path": abs_path,
            "qrcode_rel": rel,
            "message": f"请扫码登录，二维码: {rel or abs_path}",
        }
    finally:
        client.close()
...
def login(
    ...
    Returns:
        登录结果字典（含 qrcode_path / qrcode_rel）
    """
    client = XiaohongshuClient(headless=headless, cookie_path=cookie_path)
    try:
        client.start()
        ...
        rel = qrcode_repo_relative(qrcode_path) if qrcode_path else None
        if qrcode_path:
            # 等待扫码
