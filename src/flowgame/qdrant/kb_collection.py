"""FlowGame 知识库 Collection 命名：可配置前缀，_qa / _doc 区分类型。"""
...
# 知识库短名：支持中文、字母、数字、下划线、连字符（展示名，不含前缀 / _qa / _doc）
...
def normalize_kb_base_name(name: str) -> str:
    ...
    if value.startswith(prefix):
        raise KbCollectionNameError(f"请勿重复包含知识库前缀「{prefix}」")
...
def display_kb_base_name(name: str) -> str:
    """任意物理名 / base 名 → 界面展示用短名。"""
    return normalize_kb_base_name(name)


def to_qa_collection_name(base_name: str) -> str:
    return f"{get_qdrant_kb_prefix()}{normalize_kb_base_name(base_name)}{KB_QA_SUFFIX}"


def to_doc_collection_name(base_name: str) -> str:
    return f"{get_qdrant_kb_prefix()}{normalize_kb_base_name(base_name)}{KB_DOC_SUFFIX}"
...
def is_qa_collection_name(name: str) -> bool:
    prefix = get_qdrant_kb_prefix()
    n = (name or "").strip()
    return n.startswith(prefix) and n.endswith(KB_QA_SUFFIX)


def is_doc_collection_name(name: str) -> bool:
    prefix = get_qdrant_kb_prefix()
    n = (name or "").strip()
    return n.startswith(prefix) and n.endswith(KB_DOC_SUFFIX)
...
def filter_flowgame_kb_collections(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """仅保留当前前缀且带 _qa/_doc 后缀的 Collection。"""
...
def collections_for_search(name: str) -> List[Tuple[str, str]]:
    """工作流检索：当前前缀下的 {base}_qa + {base}_doc。"""
    raw = (name or "").strip()
    if not raw:
        return []
